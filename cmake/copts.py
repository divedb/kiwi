...
compilation options:
  ...
  (1) Edit the appropriate list in this file based on the platform the flag is
      needed on.
  (2) Run `<path_to_kiwi>/copts/generate_copts.py`.
...
KIWI_GCC_TEST_ADDITIONAL_FLAGS = [
...
KIWI_LLVM_TEST_ADDITIONAL_FLAGS = [
    "-Wno-deprecated-declarations",
...
COPT_VARS = {
    "KIWI_GCC_FLAGS": KIWI_GCC_FLAGS,
    "KIWI_GCC_TEST_FLAGS": GccStyleFilterAndCombine(
        KIWI_GCC_FLAGS, KIWI_GCC_TEST_ADDITIONAL_FLAGS
    ),
    "KIWI_LLVM_FLAGS": KIWI_LLVM_FLAGS,
    "KIWI_LLVM_TEST_FLAGS": GccStyleFilterAndCombine(
        KIWI_LLVM_FLAGS, KIWI_LLVM_TEST_ADDITIONAL_FLAGS
    ),
    "KIWI_CLANG_CL_FLAGS": MSVC_BIG_WARNING_FLAGS + MSVC_DEFINES,
    "KIWI_CLANG_CL_TEST_FLAGS": (
        MSVC_BIG_WARNING_FLAGS + MSVC_DEFINES + KIWI_LLVM_TEST_ADDITIONAL_FLAGS
    ),
    "KIWI_MSVC_FLAGS": (
        MSVC_BIG_WARNING_FLAGS + MSVC_WARNING_FLAGS + MSVC_DEFINES
    ),
    "KIWI_MSVC_TEST_FLAGS": (
        MSVC_BIG_WARNING_FLAGS
        + MSVC_WARNING_FLAGS
        + MSVC_DEFINES
        + [

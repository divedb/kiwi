...
template <typename T>
KIWI_ALWAYS_INLINE void compiler_must_not_elide_fn::operator()(
    T const& t) const noexcept {
  detail::compiler_must_force_sink(&t);
}

template <typename T>
KIWI_ALWAYS_INLINE void compiler_must_not_predict_fn::operator()(
    T& t) const noexcept {
  detail::compiler_must_force_sink(&t);
}
...
using compiler_must_force_indirect =
    std::bool_constant<!std::is_trivially_copyable_v<D> ||  //
                       sizeof(long) < sizeof(D) ||          //
                       std::is_pointer<D>::value>;

template <typename T>
KIWI_ALWAYS_INLINE void compiler_must_not_elide(T const& t, std::false_type) {
...
template <typename T>
KIWI_ALWAYS_INLINE void compiler_must_not_elide(T const& t, std::true_type) {
...
template <typename T>
KIWI_ALWAYS_INLINE void compiler_must_not_predict(T& t, std::false_type) {
  asm volatile("" : "+r"(t));
}

template <typename T>
KIWI_ALWAYS_INLINE void compiler_must_not_predict(T& t, std::true_type) {
  asm volatile("" : : "m"(t) : "memory");
}

...
struct compiler_must_not_elide_fn {
  template <typename T>
  KIWI_ALWAYS_INLINE void operator()(T const& t) const noexcept;
};
...
struct compiler_must_not_predict_fn {
  template <typename T>
  KIWI_ALWAYS_INLINE void operator()(T& t) const noexcept;
};
...
template <typename T>
using detect_kiwi_is_unsafe_for_async_usage =
    typename T::kiwi_is_unsafe_for_async_usage;
...
inline constexpr bool is_unsafe_for_async_usage_v =
    detected_or_t<std::false_type,
                  detail::detect_kiwi_is_unsafe_for_async_usage, T>::value;

}  // namespace detail

//  unsafe_for_async_usage
//
//  Defines member type alias kiwi_is_unsafe_for_async_usage, which is the tag
...
//    * In particular, the storage defined by KIWI_DECLARE_REUSED.
...
//    `using kiwi_is_unsafe_for_async_usage = std::true_type`.
...
//    KIWI_ATTR_NO_UNIQUE_ADDRESS, to avoid increasing the size of the class.
//  * std::lock_guard/unique_lock/scoped_lock are considered unsafe unless the
//    underlying mutex has a typedef `kiwi_coro_aware_mutex`.
//
// NOTE: you can explicitly opt out from a check for a type by having
//       `kiwi_is_unsafe_for_async_usage` be `std::false_type`.
//
...
template <typename T>
using detect_kiwi_is_coro_aware_mutex = typename T::kiwi_coro_aware_mutex;
...
// Detects the presense of kiwi_coro_aware_mutex nested typedef.
// This helps custom lock guards have the same behavior as std::lock_guard.
template <typename T>
constexpr bool is_coro_aware_mutex_v =
    is_detected_v<detail::detect_kiwi_is_coro_aware_mutex, T>;

}  // namespace kiwi

#include "kiwi/coro/hint-inl.hh"

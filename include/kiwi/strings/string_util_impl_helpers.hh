...
template <typename T, typename CharT = typename T::value_type>
TrimPositions TrimStringT(T input, T trim_chars, TrimPositions positions,
...
std::basic_string<CharT> CollapseWhitespaceT(
    T text, bool trim_sequences_with_line_breaks) {
...
bool DoReplaceMatchesAfterOffset(std::basic_string<CharT>* str,
                                 size_t initial_offset, Matcher matcher,
                                 T replace_with, ReplaceType replace_type) {
...
template <typename T, typename CharT = typename T::value_type>
bool ReplaceCharsT(T input, T find_any_of_these, T replace_with,
                   std::basic_string<CharT>* output) {
...
std::optional<std::basic_string<CharT>> DoReplaceStringPlaceholders(
    T format_string, Range&& subst, const CharT placeholder_prefix,
    const bool should_escape_multiple_placeholder_prefixes,
    const bool is_strict_mode, std::vector<size_t>* offsets) {

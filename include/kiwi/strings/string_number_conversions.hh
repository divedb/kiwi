...
// Instead, use kiwi::MessageFormatter for a complex message with numbers
// (integer, float, double) embedded or kiwi::Format{Number,Double,Percent} to
...
BASE_EXPORT std::string HexEncode(kiwi::span<const uint8_t> bytes);
...
// routines.
inline void AppendHexEncodedByte(uint8_t byte, std::string& output,
                                 bool uppercase = true) {
...
BASE_EXPORT bool HexStringToSpan(std::string_view input,
                                 kiwi::span<uint8_t> output);

}  // namespace kiwi

#if BUILDFLAG(IS_WIN)
#include "kiwi/strings/string_number_conversions_win.hh"
#endif

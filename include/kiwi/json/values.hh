...
class BASE_EXPORT GSL_OWNER ListValue {
 ...
 private:
  ...
  bool contains(const T& val, bool (Value::*test)() const,
...
class BASE_EXPORT GSL_OWNER Value {
 ...
 private:
  ...
  std::variant<std::monostate, bool, int, DoubleStorage, std::string,
               BlobStorage, Dict, List>
      data_;
};
...
class BASE_EXPORT GSL_POINTER ValueView {
 ...
 private:
  using ViewType =
      std::variant<std::monostate, bool, int, Value::DoubleStorage,
                   std::string_view,
...
template <typename T, typename R>
  requires std::equality_comparable_with<T, R>
bool ListValue::contains(const T& val, bool (Value::*test)() const,
                         R (Value::*get)() const) const {

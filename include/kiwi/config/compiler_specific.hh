...
/// stacks. Often useful with `NOINLINE` to make sure the function itself is
...
/// The compiler will check that the provided arguments match the type
/// specifiers in the format string. Useful to detect mismatched format
/// strings/args.
///
...
/// fully initialized. Useful for e.g. code that deliberately reads
/// uninitialized data, such as a GC scavenging root set pointers from the
/// stack.
///
/// See also:
...
/// NOTE: Prefer `DISABLE_CFI_DLSYM()` if you just need to allow calling of
/// dlsym functions.
///
/// See also:
...
/// Annotates a function disabling Control Flow Integrity indirect call checks
/// if doing so is necessary to call dlsym functions. The checks are retained on
...
/// parts of all local variables, to mitigate security risks. In most cases,
/// e.g. when such memory is either never accessed or will be initialized later
/// before reading, the compiler is able to remove the additional stores, and
/// any remaining stores are unlikely to affect program performance.
///
...
/// `memcpy()`/`memset()` calls to init variables. If this causes link errors
/// for targets that don't link against the CRT, this macro can help; you may
/// instead want 'configs -= [ "//build/config/compiler:default_init_stack_vars"
/// ]' in the relevant .gn file to disable this on the whole target.
///
/// See also:
...
/// Annotates a codepath suppressing static analysis along that path. Useful
/// when code is safe in practice for reasons the analyzer can't detect, e.g.
/// because the condition leading to that path guarantees a param is non-null.
///
/// Usage:
/// ```
///   if (cond) {
...
/// optimizations that merge calls. Useful to ensure the source locations of
/// such calls are not obscured.
///
/// See also:
...
/// not be concerned about its address remaining constant, be safely usable
/// after copying its memory, and have a destructor that may be safely omitted
/// on moved-from instances; an example is `std::unique_ptr`. Unnecessary if the
...
/// Usage:
/// ```
///   // Instances of type `S` will be eligible to be passed in registers
///   // despite`S`'s nontrivial destructor.
...
///     // This block will only be executed if type `T` is trivially
///     relocatable.
///   }
/// ```
...
/// `char` and vends its address via `.data()`. Used to detect lifetime errors
/// in conjunction with `GSL_POINTER`; see documentation there.
///
/// See also:
...
/// "Mutable Constants" trybot check [1] when logically const instances are
/// named like `kConstants` but for some reason should not be marked `const`.
///
/// [1]:
...
/// caller performance in the common case, at the cost of pessimizing the
/// callee. On other platforms, this attribute has no effect as of Clang 20.
///
/// See also:
...
/// destroyed, the called-on object is also considered to be dangling. For
/// return values, the value is assumed to point into the called-on object, so
/// if that object is destroyed, the returned value is also considered to be
/// dangling. Useful to diagnose some cases of lifetime errors.
///
/// See also:
...
///
/// Any weak definition of a symbol will be overridden at link time by a
/// non-weak definition. Marking a `const` or `constexpr` variable weak makes it
/// no longer be considered a compile-time constant, since its value may be
/// different after linking.
///
...
///
///   // The following definition provides a default implementation of
///   // `Func2()`, but allows other object files to override.
...
/// Annotates a type or member indicating the minimum possible alignment (one
/// bit for bitfields, one byte otherwise) should be used. This can be used to
/// eliminate padding inside objects, at the cost of potentially pessimizing
/// code, or even generating invalid code (depending on platform restrictions)
/// if underaligned objects have their addresses taken and passed elsewhere.
///
...
/// Annotates a function indicating that the returned pointer will never be
/// null. This may allow the compiler to assume null checks on the caller side
/// are unnecessary.
///
...
/// Annotates a function indicating it is pure, meaning that it has no
/// observable side effects. Unlike functions annotated `CONST_FUNCTION`, pure
/// functions may still read external memory, and thus their return values may
/// change between calls. `strlen()` and `memcmp()` are examples of pure
/// functions. Useful to allow folding/reordering calls for optimization
/// purposes.
///
/// WARNING: Misusing this attribute can lead to silent miscompilation, UB, and
/// difficult-to-diagnose bugs. Because apparently-safe invocations can
/// sometimes have side effects (especially when invoking "overridable"
/// functionality like virtual or templated methods), such misuse is far more
/// likely than it seems. Therefore, this macro should generally be used only in
/// key vocabulary types, where the perf and ergonomic benefits of callers not
/// needing to worry about caching results in local variables in hot code
/// outweighs the risks.
///
/// See also:
...
///
/// Annotating a function `UNSAFE_BUFFER_USAGE` means all call sites (that do
/// not disable the warning) must wrap calls in `UNSAFE_BUFFERS()`; see
/// documentation there. Annotating a field `UNSAFE_BUFFER_USAGE` means that
/// `UNSAFE_BUFFERS()` must wrap expressions that mutate of the field.
///
/// See also:
...
/// compile-time errors if functions' arguments are always known at compile
/// time.
///
/// SFINAE and `requires` clauses can restrict function availability based on
/// the unevaluated context (type information and syntactic correctness). This
/// provides a similar capability based on the evaluated context (variable
/// values). If the condition fails, or cannot be determined at compile time,
/// the function is excluded from the overload set.
///
...
///     // `ENABLE_IF_ATTR(i > 0, ...)` will not help; the compiler's analysis
///     is

...
// Example:
//
//   kiwi::FileEnumerator e(my_dir, false, kiwi::FileEnumerator::FILES,
//                          FILE_PATH_LITERAL("*.txt"));
// Using `ForEach` with a lambda:
//   e.ForEach([](const kiwi::FilePath& item) {...});
// Using a `for` loop:
//   for (kiwi::FilePath name = e.Next(); !name.empty(); name = e.Next())
//     ...
class BASE_EXPORT FileEnumerator {
 public:
  ...
  class BASE_EXPORT FileInfo {
   public:
    ...
    FileInfo(kiwi::FilePath content_uri, kiwi::FilePath filename,
             bool is_directory, off_t size, Time time);
#endif
 ...
 private:
  ...
  kiwi::stack<FilePath> pending_paths_;
#if BUILDFLAG(IS_ANDROID)
  // Matches pending_paths_, but with display names.
  kiwi::stack<std::vector<std::string>> pending_subdirs_;

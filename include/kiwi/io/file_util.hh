...
/// and references to /./ and /../ in file_name.  If the resolved_name argument
/// is non-NULL, the resulting absolute pathname is copied there (it must refer
/// to a buffer capable of storing at least PATH_MAX characters).
///
...
///
/// \param input The input path, which can be relative or absolute.
/// \return A FilePath representing the absolute (canonicalized) version of the
///         input.
...
/// Returns the total number of bytes used by all the files under \p root_path.
///
/// This function is implemented using the FileEnumerator class so it is not
/// particularly speedy on any platform.
///
/// \param root_path The root directory whose size (including all subdirectories
///                  and files) is to be calculated.
/// \return The total size in bytes of all files under the given directory path.
///         Returns -1 if an error occurs (e.g., permission denied).
...
///
/// In POSIX environment and if \p path is a symbolic link, this deletes only
/// the symlink. (even if the symlink points to a non-existent file)
///
/// \param path The file or directory path to be deleted.
...
BASE_EXPORT bool ReadStreamToStringWithMaxSize(FILE* stream, size_t max_size,
                                               std::string* contents);
...
/// Ensures a directory exists, if necessary, creating it and parent
/// directories. Returns true if the directory already existed or was created,
/// leaving *error unchanged. Returns false on failure and sets *error
/// appropriately if it is non-NULL.
///
...
BASE_EXPORT bool TouchFile(const FilePath& path, const Time& last_accessed,
...
BASE_EXPORT bool PreReadFile(
    const FilePath& file_path, bool is_executable, bool sequential,
...
BASE_EXPORT bool CopyFileContentsWithSendfile(File& infile, File& outfile,
                                              bool& retry_slow);

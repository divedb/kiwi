...
template <typename ByteType, typename ElementType, size_t Extent,
...
class GSL_POINTER span {
 public:
  ...
  friend constexpr bool operator==(
      span lhs, span<OtherElementType, OtherExtent, OtherInternalPtrType> rhs) {
    return std::ranges::equal(span<const element_type, extent>(lhs),
...
class GSL_POINTER span<ElementType, dynamic_extent, InternalPtrType> {
 public:
  ...
  // NOLINTNEXTLINE(google-explicit-constructor)
  constexpr span(const span<OtherElementType, OtherExtent,
                            OtherInternalPtrType>& other) noexcept
...
constexpr std::ostream& operator<<(
    std::ostream& l, span<ElementType, Extent, InternalPtrType> r) {
  l << '[';

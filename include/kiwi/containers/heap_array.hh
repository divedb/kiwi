...
class TRIVIAL_ABI GSL_OWNER HeapArray {
 public:
  ...
  using iterator = kiwi::span<T>::iterator;
  using const_iterator = kiwi::span<const T>::iterator;
  ...
  static HeapArray CopiedFrom(kiwi::span<const T> that) {
  ...
  void copy_from(kiwi::span<const T> other) { as_span().copy_from(other); }
  ...
  void copy_prefix_from(kiwi::span<const T> other) {
  ...
  kiwi::HeapArray<T> take_first(size_t reduced_size) && {
    CHECK_LE(reduced_size, size_);
    size_ = 0u;
    if (!reduced_size) {
      data_.reset();
    }
    return kiwi::HeapArray(std::move(data_), reduced_size);
  }

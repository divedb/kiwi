...
KIWI_HAS_TRUE_XXX(IsRelocatable);
KIWI_HAS_TRUE_XXX(IsZeroInitializable);

#undef KIWI_HAS_TRUE_XXX
 ...
 * template <> KIWI_ASSUME_RELOCATABLE(MyType)
 *
 * When using it with a template type, use it like this:
 *
 * // Make sure you're at namespace ::folly scope
 * template <class T1, class T2>
 * KIWI_ASSUME_RELOCATABLE(MyType<T1, T2>)
 */
#define KIWI_ASSUME_RELOCATABLE(...) \
  struct IsRelocatable<__VA_ARGS__> : std::true_type {}

/**
 * The KIWI_ASSUME_FBVECTOR_COMPATIBLE* macros below encode the
 ...
 *
 * Use KIWI_ASSUME_FBVECTOR_COMPATIBLE with regular types like this:
 *
 * KIWI_ASSUME_FBVECTOR_COMPATIBLE(MyType)
 *
 * The versions KIWI_ASSUME_FBVECTOR_COMPATIBLE_1, _2, _3, and _4
 ...
 * KIWI_ASSUME_FBVECTOR_COMPATIBLE_2(MyType)
 */
...
KIWI_PUSH_WARNING
KIWI_GNU_DISABLE_WARNING("-Wsign-compare")
KIWI_GCC_DISABLE_WARNING("-Wbool-compare")
KIWI_MSVC_DISABLE_WARNING(4287)  // unsigned/negative constant mismatch
KIWI_MSVC_DISABLE_WARNING(4388)  // sign-compare
KIWI_MSVC_DISABLE_WARNING(4804)  // bool-compare
...
#ifndef _MSC_VER
KIWI_ASSUME_FBVECTOR_COMPATIBLE_2(std::unique_ptr)
...
#if KIWI_HAS_BUILTIN(__type_pack_element)
 ...
 * ```
 */
template <typename T, template <typename U> typename Hasher,

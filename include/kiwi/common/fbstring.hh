...
/// Copies exactly `n` values from the range begining at `b` to the range
/// beginning at `d`. Formally, for each integer `i` in [0, n), performs
/// `*(d + i) = *(b + i)`.
/// Overlap of ranges is formally permitted, but leads to unpredictable
/// ordering of the results.
///
/// Returns: A pair of iterators, where the first points to the position in
/// the input range after the last copied element, and the second points to
/// the position in the output range after the last written element.
...
///   This scheme ensures that when `size == `maxSmallSize`, the last byte in
///   the storage is \0. This way, storage will be a null-terminated sequence of
...
class fbstring_core {
 public:
  ...
  fbstring_core(const fbstring_core& rhs) {
    ...
    switch (rhs.Category()) {
      case Category::kIsSmall:
        CopySmall(rhs);
        break;
      case Category::kIsMedium:
        CopyMedium(rhs);
        break;
      case Category::kIsLarge:
        CopyLarge(rhs);
        break;
      default:
        kiwi::unreachable();
  ...
  fbstring_core(const Char* const data, const size_t size,
                bool disable_sso = FBSTRING_DISABLE_SSO) {
    if (!disable_sso && size <= kMaxSmallSize) {
      InitSmall(data, size);
  ...
  fbstring_core(Char* const data, const size_t size,
                const size_t allocated_size, AcquireMallocatedString) {
    if (size > 0) {
      assert(allocated_size >= size + 1);
      assert(data[size] == '\0');
      // Use the medium string storage
      ml_.data_ = data;
      ml_.size_ = size;
      // Don't forget about null terminator
      ml_.set_capacity(allocated_size - 1, Category::kIsMedium);
    } else {
  ...
  const Char* c_str() const {
    ...
    ptr = (Category() == Category::kIsSmall) ? small_ : ptr;
  ...
  void Shrink(const size_t delta) {
    if (Category() == Category::kIsSmall) {
      ShrinkSmall(delta);
    } else if (Category() == Category::kIsMedium ||
 ...
 private:
  ...
  enum class Category : category_type {
    kIsSmall = 0,
    kIsMedium = kIsLittleEndian ? 0x80 : 0x2,
    kIsLarge = kIsLittleEndian ? 0x40 : 0x1,
  };

  Category Category() const {
    // Works for both big-endian and little-endian.
    return static_cast<Category>(bytes_[kLastChar] & categoryExtractMask);
  }
  ...
  constexpr static uint8_t kCategoryExtractMask = kIsLittleEndian ? 0xC0 : 0x3;
  constexpr static size_t kCategoryShift = (sizeof(size_t) - 1) * 8;
  constexpr static size_t kCapacityExtractMask =
      kIsLittleEndian ? ~(size_t(kCategoryExtractMask) << kCategoryShift)
                      : 0x0 /* unused */;

  static_assert(!(sizeof(MediumLarge) % sizeof(Char)),
                "Corrupt memory layout for fbstring.");

  /// TODO(gc): what is good malloc size?
  size_t good_malloc_size(size_t min_size) noexcept { return min_size; }
...
inline void fbstring_core<Char>::CopySmall(const fbstring_core& rhs) {
  ...
  // ml_.capacity field).
  ml_ = rhs.ml_;
  assert(category() == Category::kIsSmall && this->size() == rhs.size());
}
...
void fbstring_core<Char>::CopyMedium(const fbstring_core& rhs) {
  ...
  auto const alloc_size = good_malloc_size((1 + rhs.ml_.size_) * sizeof(Char));
  ml_.data_ = static_cast<Char*>(checked_malloc(allocSize));
...
// compiling with ASan.
  ...
  if ((reinterpret_cast<size_t>(data) & (sizeof(size_t) - 1)) == 0) {
    const size_t byte_size = size * sizeof(Char);
    constexpr size_t word_width = sizeof(size_t);
...
//   const_reference; typedef typename std::allocator_traits<A>::pointer
//   pointer; typedef typename std::allocator_traits<A>::const_pointer
//   const_pointer;
...
//   FOLLY_NOINLINE basic_fbstring(string_view_type view, const A&,
//                                 string_view_ctor)
...
//   basic_fbstring(const basic_fbstring& str, size_type pos, size_type n =
//   npos,
//                  const A& /* a */ = A()) {
//     assign(str, pos, n);
//   }
...
//               !std::is_convertible_v<const StringViewLike&, const
//               value_type*>,
//           int> = 0>
...
//               value_type*>,
//           int> = 0>
//   basic_fbstring(const StringViewLike& view, size_type pos, size_type n,
//                  const A& a = A())
//       : basic_fbstring(string_view_type(view).substr(pos, n), a,
//                        string_view_ctor{}) {}
...
//  public:
//   template <class ItOrLength, class ItOrChar>
//   iterator insert(const_iterator p, ItOrLength first_or_n, ItOrChar
//   last_or_c) {
//     using Sel =
...
//   basic_fbstring& replace(iterator i1, iterator i2, const basic_fbstring&
//   str) {
...
//                                    InputIter e, std::integral_constant<int,
//                                    0>);
...
//   template <class FwdIterator>
//   bool replaceAliased(iterator i1, iterator i2, FwdIterator s1, FwdIterator
//   s2,
//                       std::true_type);
...
//   template <class InputIterator>
//   void replaceImpl(iterator i1, iterator i2, InputIterator b, InputIterator
//   e,
//                    std::input_iterator_tag);
...
// template <typename E, class T, class A, class S>
// FOLLY_NOINLINE basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A,
// S>::append(
//     const value_type* s, size_type n) {
//   Invariant checker(*this);
...
// basic_fbstring<E, T, A, S>::find(const value_type* needle, const size_type
// pos,
//                                  const size_type nsize) const {
//   auto const size = this->size();
//   // nsize + pos can overflow (eg pos == npos), guard against that by
//   checking
...
// template <typename E, class T, class A, class S>
// inline basic_fbstring<E, T, A, S>& basic_fbstring<E, T, A,
// S>::replaceImplDiscr(
...
// template <typename E, class T, class A, class S>
// template <class FwdIterator>
// inline bool basic_fbstring<E, T, A, S>::replaceAliased(iterator i1, iterator
// i2,
//                                                        FwdIterator s1,
//                                                        FwdIterator s2,
//                                                        std::true_type) {
...
// template <typename E, class T, class A, class S>
// inline typename basic_fbstring<E, T, A, S>::size_type
// basic_fbstring<E, T, A, S>::find_last_not_of(const value_type* s, size_type
// pos,
//                                              size_type n) const {
//   if (!this->empty()) {
...
//                        const basic_fbstring<E, T, A, S>&) = delete;
...
// template <typename E, class T, class A, class S>
// inline bool operator==(const basic_fbstring<E, T, A, S>&,

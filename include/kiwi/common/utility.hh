...
template <typename T>
auto decay_0_(int) -> decltype(detail::decay_1_(KIWI_DECLVAL(T&&)));
template <typename T>
...
struct inheritable_inherit_ : T {
  ...
  /* implicit */ KIWI_ERASE inheritable_inherit_(A&&... a) noexcept(
...
struct inheritable_contain_ {
  ...
  template <typename... A,
            std::enable_if_t<std::is_constructible<T, A...>::value, int> = 0>
  /* implicit */ KIWI_ERASE inheritable_contain_(A&&... a) noexcept(
      noexcept(T(static_cast<A&&>(a)...)))
      : v(static_cast<A&&>(a)...) {}
  KIWI_ERASE operator T&() & noexcept { return v; }
  KIWI_ERASE operator T&&() && noexcept { return static_cast<T&&>(v); }
  KIWI_ERASE operator T const&() const& noexcept { return v; }
  KIWI_ERASE operator T const&&() const&& noexcept {
...
struct unsafe_default_initialized_cv {
  ...
  KIWI_MSVC_DISABLE_WARNING(4700)
  // Potentially uninitialized local variable 'uninit' used
  KIWI_MSVC_DISABLE_WARNING(4701)
  // Potentially uninitialized local pointer variable 'uninit' used
  KIWI_MSVC_DISABLE_WARNING(4703)
  KIWI_GNU_DISABLE_WARNING("-Wuninitialized")
  // Clang doesn't implement -Wmaybe-uninitialized and warns about it
  KIWI_GCC_DISABLE_WARNING("-Wmaybe-uninitialized")
  template <typename T>
  KIWI_ERASE constexpr /* implicit */ operator T() const noexcept {
...
class to_narrow_convertible {
 ...
 public:
  ...
  /* implicit */ constexpr operator Dst() const noexcept {
    KIWI_PUSH_WARNING
    KIWI_MSVC_DISABLE_WARNING(4244)  // lossy conversion: arguments
    KIWI_MSVC_DISABLE_WARNING(4267)  // lossy conversion: variables
    KIWI_GNU_DISABLE_WARNING("-Wconversion")
    return value_;
    KIWI_POP_WARNING
  }
...
namespace detail {
template <typename R>
using invocable_to_detect = decltype(KIWI_DECLVAL(R)());
...
class invocable_to_convertible : private inheritable<F> {
 private:
  ...
  template <typename R>
  static constexpr bool nx_v = noexcept(KIWI_DECLVAL(R)());
  template <typename G>
...
struct invocable_to_fn {
  ...
  KIWI_ERASE constexpr R operator()(F&& f) const

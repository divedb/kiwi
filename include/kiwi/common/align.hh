...
#if BUILDFLAG(IS_WIN)
inline constexpr std::size_t kRegisterPassMaxSize = 8u;
#else
inline constexpr std::size_t kRegisterPassMaxSize = 16u;
#endif
...
constexpr bool is_register_pass_v =
    (sizeof(T) <= kRegisterPassMaxSize) && std::is_trivially_copyable_v<T>;
...
inline constexpr bool kHasExtendedAlignment =
    sizeof(void*) >= sizeof(std::uint64_t);
#else
inline constexpr bool kHasExtendedAlignment = false;
#endif
...
///
/// You should refer to this as `::kiwi::max_align_t` in portable code, even if
/// you have `using namespace kiwi;` because C11 defines a global namespace
/// `max_align_t` type.
///
...
/// `long double`.
///
/// So far we've covered locals and other non-allocated storage, but we also
/// need confidence that allocated storage from `malloc`, `new`, etc will also
/// be suitable for objects with this alignment requirement.
///
...
///
/// We also have some anecdotal evidence: we don't see lots of
/// misaligned-storage crashes on 32-bit iOS apps that use `double`.
///
/// Apple's allocation reference: http://bit.ly/malloc-small
inline constexpr std::size_t kMaxAlign = detail::max_align_v_::value();
struct alignas(kMaxAlign) MaxAlignType {};
...
#if defined(ARCH_CPU_S390_FAMILY) || defined(ARCH_CPU_ARM_FAMILY)
...
static_assert(kHardwareConstructiveInterferenceSize >= kMaxAlign, "math?");
...
inline constexpr std::size_t kCacheLineAlign =
    kHasExtendedAlignment ? kHardwareConstructiveInterferenceSize : kMaxAlign;
struct alignas(kCacheLineAlign) CacheLineAlignType {};

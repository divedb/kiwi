...
struct CacheLocality {
  ...
  ///
  /// \param mapping A callback function that takes a sysfs path string.
  /// \return A CacheLocality object.
  ...
  /// layer.  num_caches_by_level.front() is the number of L1 caches and
  /// num_caches_by_level.back() is the number of last-level caches.
  ///
  /// Suppose a system has the following cache hierarchy:
  /// - 8 logical CPUs
  /// - Each core has its own L1 cache
  /// - Every 2 cores share one L2 cache
  /// - All cores share a single L3 cache
  /// num_caches_by_level = {8, 4, 1};
  std::vector<size_t> num_caches_by_level;

  /// A map from cpu (from sched_getcpu or getcpu) to an index in the
  /// range 0..num_cpus-1, where neighboring locality indices are more
 ...
 private:
  explicit CacheLocality(std::vector<std::vector<size_t>> equiv_classes);
};

...
using SynchronizedLockPolicyExclusive =
    SynchronizedLockPolicy<SynchronizedMutexLevel::Unique,
...
using SynchronizedLockPolicyShared =
    SynchronizedLockPolicy<SynchronizedMutexLevel::Shared,
                           SynchronizedMutexMethod::Lock>;
using SynchronizedLockPolicyTryShared =
    SynchronizedLockPolicy<SynchronizedMutexLevel::Shared,
                           SynchronizedMutexMethod::TryLock>;
using SynchronizedLockPolicyUpgrade =
    SynchronizedLockPolicy<SynchronizedMutexLevel::Upgrade,
                           SynchronizedMutexMethod::Lock>;
...
class SynchronizedBase<Subclass, detail::SynchronizedMutexLevel::Upgrade>
    : public SynchronizedBase<Subclass,
...
struct Synchronized
    : public SynchronizedBase<Synchronized<T, Mutex>,
 ...
 public:
  ...
  /* implicit */ Synchronized(
      typename std::conditional<std::is_copy_constructible<T>::value,
                                const Synchronized&, NonImplementedType>::type
          rhs) /* may throw */
  ...
  Synchronized(std::piecewise_construct_t, std::tuple<DatumArgs...> datumArgs,
               std::tuple<MutexArgs...> mutexArgs)
      : Synchronized{std::piecewise_construct, std::move(datumArgs),
                     std::move(mutexArgs),
 ...
 private:
  ...
  Synchronized(Synchronized&& rhs,
               const LockedPtr& /*guard*/) noexcept(nxMoveCtor)
...
using SynchronizedDataType =
    typename std::conditional<AllowsConcurrentAccess ||
...
using LockedPtrType =
    typename std::conditional<std::is_const<SynchronizedType>::value,
                              typename SynchronizedType::ConstLockedPtr,
                              typename SynchronizedType::LockedPtr>::type;

template <typename Synchronized, typename LockFunc, typename TryLockFunc,
          typename... Args>
class SynchronizedLocker {
 public:
  ...
  SynchronizedLocker(Synchronized& sync, LockFuncType&& lockFunc,
                     TryLockFuncType tryLockFunc, As&&... as)
      : synchronized{sync},
...
template <typename Synchronized, typename LockFunc, typename TryLockFunc,
          typename... Args>
auto makeSynchronizedLocker(Synchronized& synchronized, LockFunc&& lockFunc,
                            TryLockFunc&& tryLockFunc, Args&&... args) {
  using LockFuncType = std::decay_t<LockFunc>;
  using TryLockFuncType = std::decay_t<TryLockFunc>;
  return SynchronizedLocker<Synchronized, LockFuncType, TryLockFuncType,
                            std::decay_t<Args>...>{
      synchronized, std::forward<LockFunc>(lockFunc),
      std::forward<TryLockFunc>(tryLockFunc), std::forward<Args>(args)...};
}
...
void lock(LockableOne& one, LockableTwo& two, Lockables&... lockables) {
  auto locker = [](auto& lockable) {
    ...
    return detail::makeSynchronizedLocker(
        lockable, [](auto& l) { return std::unique_lock<Lockable>{l}; },
        [](auto& l) {
...
#define SYNCHRONIZED(...)                                                 \
  ...
  if (bool SYNCHRONIZED_VAR(state) = false) {                             \
    ...
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                               \
         ...
         !SYNCHRONIZED_VAR(state); SYNCHRONIZED_VAR(state) = true)        \

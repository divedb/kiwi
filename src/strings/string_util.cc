...
const std::string& EmptyString() {
  static const kiwi::NoDestructor<std::string> s;
  return *s;
}

const std::u16string& EmptyString16() {
  static const kiwi::NoDestructor<std::u16string> s16;
  return *s16;
}

bool ReplaceChars(std::u16string_view input, std::u16string_view replace_chars,
                  std::u16string_view replace_with, std::u16string* output) {
  return internal::ReplaceCharsT(input, replace_chars, replace_with, output);
}

bool ReplaceChars(std::string_view input, std::string_view replace_chars,
                  std::string_view replace_with, std::string* output) {
  return internal::ReplaceCharsT(input, replace_chars, replace_with, output);
}

bool RemoveChars(std::u16string_view input, std::u16string_view remove_chars,
                 std::u16string* output) {
  return internal::ReplaceCharsT(input, remove_chars, std::u16string_view(),
                                 output);
}

bool RemoveChars(std::string_view input, std::string_view remove_chars,
                 std::string* output) {
  return internal::ReplaceCharsT(input, remove_chars, std::string_view(),
                                 output);
}

bool TrimString(std::u16string_view input, std::u16string_view trim_chars,
                std::u16string* output) {
  return internal::TrimStringT(input, trim_chars, TRIM_ALL, output) !=
         TRIM_NONE;
}

bool TrimString(std::string_view input, std::string_view trim_chars,
                std::string* output) {
  return internal::TrimStringT(input, trim_chars, TRIM_ALL, output) !=
         TRIM_NONE;
}
...
void TruncateUTF8ToByteSize(std::string_view input, const size_t byte_size,
                            std::string* output) {
  // TODO(gc):
  // DCHECK(output);
...
TrimPositions TrimWhitespace(std::u16string_view input, TrimPositions positions,
                             std::u16string* output) {
...
std::u16string FormatBytesUnlocalized(int64_t bytes) {
  ...
  if (bytes != 0 && dimension > 0 && unit_amount < 100) {
    UNSAFE_TODO(kiwi::snprintf(buf, std::size(buf), "%.1lf%s", unit_amount,
                               kByteStringsUnlocalized[dimension]));
  } else {
    UNSAFE_TODO(kiwi::snprintf(buf, std::size(buf), "%.0lf%s", unit_amount,
                               kByteStringsUnlocalized[dimension]));
  }
...
void ReplaceFirstSubstringAfterOffset(std::u16string* str, size_t start_offset,
...
void ReplaceFirstSubstringAfterOffset(std::string* str, size_t start_offset,
...
void ReplaceSubstringsAfterOffset(std::u16string* str, size_t start_offset,
                                  std::u16string_view find_this,
                                  std::u16string_view replace_with) {
  internal::DoReplaceMatchesAfterOffset(
      str, start_offset, internal::MakeSubstringMatcher(find_this),
...
void ReplaceSubstringsAfterOffset(std::string* str, size_t start_offset,
                                  std::string_view find_this,
                                  std::string_view replace_with) {
  internal::DoReplaceMatchesAfterOffset(
      str, start_offset, internal::MakeSubstringMatcher(find_this),
...
std::u16string ReplaceStringPlaceholders(std::u16string_view format_string,
                                         kiwi::span<const std::u16string> subst,
...
std::string ReplaceStringPlaceholders(std::string_view format_string,
                                      kiwi::span<const std::string> subst,
                                      std::vector<size_t>* offsets) {
...
size_t strlcpy(char* dst, const char* src, size_t dst_size) {
  return internal::lcpyT(
      UNSAFE_TODO(kiwi::span(dst, dst_size), std::string_view(src)));
}

size_t u16cstrlcpy(char16_t* dst, const char16_t* src, size_t dst_size) {
  return internal::lcpyT(UNSAFE_TODO(kiwi::span(dst, dst_size)),

...
bool ReadUnicodeCharacter(const char16_t* src, size_t src_len,
                          size_t* char_index, base_icu::UChar32* code_point) {
...
bool ReadUnicodeCharacter(const wchar_t* src, size_t src_len,
                          size_t* char_index, base_icu::UChar32* code_point) {
...
void PrepareForUTF8Output(const CHAR* src, size_t src_len,
                          std::string* output) {
  output->clear();
  if (src_len == 0) {
    return;
...
void PrepareForUTF16Or32Output(const char* src, size_t src_len,
                               STRING* output) {
  output->clear();
  if (src_len == 0) {
    return;

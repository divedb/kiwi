...
#include "kiwi/strings/string_number_conversions.hh"
...
void RunAndReply(Function<bool()> action_callback,
                 Function<void(bool)> reply_callback) {
  bool result = std::move(action_callback)();

  if (reply_callback) {
    std::move(reply_callback)(result);
  }
}
...
bool ReadStreamToSpanWithMaxSize(
  ...
  // size is small (i.e. proc files). If the file is larger than this, the
  // read loop will reset |chunk_size| to kDefaultChunkSize.
...
Function<void()> GetDeleteFileCallback(const FilePath& path,
                                       Function<void(bool)> reply_callback) {
  // TODO(gc):
  return {};
...
Function<void()> GetDeletePathRecursivelyCallback(
...
bool ReadStreamToStringWithMaxSize(FILE* stream, size_t max_size,
                                   std::string* contents) {
  if (contents) {
    contents->clear();
  }
...
Function<std::optional<int64_t>()> GetFileSizeCallback(const FilePath& path) {
  return [path]() -> std::optional<int64_t> { return GetFileSize(path); };
}

bool TouchFile(const FilePath& path, const Time& last_accessed,
               const Time& last_modified) {
  uint32_t flags = File::kFlagOpen | File::kFlagWriteAttributes;

#if BUILDFLAG(IS_WIN)
  // On Windows, FILE_FLAG_BACKUP_SEMANTICS is needed to open a directory.
  if (DirectoryExists(path)) {
    flags |= File::kFlagWinBackupSemantics;
  }
  ...
  flags |= (DirectoryExists(path) ? File::kFlagRead : File::kFlagWrite);
#endif
...
std::optional<uint64_t> ReadFile(const FilePath& filename,
                                 span<uint8_t> buffer) {
  return ReadFile(filename, kiwi::as_writable_chars(buffer));
}
...
FilePath GetUniquePathWithSuffixFormat(const FilePath& path,
                                       kiwi::cstring_view suffix_format) {
  DCHECK(!path.empty());
  DCHECK_EQ(std::ranges::count(suffix_format, '%'), 1);
  DCHECK(kiwi::Contains(suffix_format, "%d"));
  ...
  for (int count = 1; count <= kMaxUniqueFiles; ++count) {
    ...
    kiwi::ReplaceFirstSubstringAfterOffset(&suffix, 0, "%d",
                                           kiwi::NumberToString(count));

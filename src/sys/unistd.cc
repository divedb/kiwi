...
static_assert(sizeof(off_t) >= 8,
              "We expect that Mac OS have at least a 64-bit off_t.");

off64_t lseek64(int fh, off64_t off, int orig) { return lseek(fh, off, orig); }
...
off_t lseek(int fh, off_t off, int orig) { return _lseek(fh, off, orig); }

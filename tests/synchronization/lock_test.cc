...
template <typename UniqueLockState, typename SharedLockState,
 ...
 private:
  ...
  struct s_ {
    ...
    /* implicit */ s_(S const& s) {
      !!s == V ? void() : kiwi::throw_exception<MismatchError>();
    }
  };
...
class unique_lock<Mutex<X, S, U>>
    : public kiwi::unique_lock_base<Mutex<X, S, U>> {
  using kiwi::unique_lock_base<Mutex<X, S, U>>::unique_lock_base;
};

template <typename X, typename S, typename U>
class shared_lock<Mutex<X, S, U>>
    : public kiwi::shared_lock_base<Mutex<X, S, U>> {
  using kiwi::shared_lock_base<Mutex<X, S, U>>::shared_lock_base;
};

template <typename X, typename S, typename U>
class lock_guard<Mutex<X, S, U>>
    : public kiwi::unique_lock_guard_base<Mutex<X, S, U>> {
  using kiwi::unique_lock_guard_base<Mutex<X, S, U>>::unique_lock_guard_base;
};
...
using param_mutex_t =
    Mutex<typename Param::unique_lock_state, typename Param::shared_lock_state,
          typename Param::upgrade_lock_state>;
template <typename Param>
using param_lock_t = typename Param::template lock_type<param_mutex_t<Param>>;
template <typename Param>
using param_state_t = kiwi::detail::lock_state_type_of_t<param_lock_t<Param>>;
template <typename Param>
...
template <typename M>
static constexpr Held held_v<kiwi::unique_lock<M>> = Held::Unique;
template <typename M>
static constexpr Held held_v<kiwi::shared_lock<M>> = Held::Shared;
template <typename M>
static constexpr Held held_v<kiwi::upgrade_lock<M>> = Held::Upgrade;

template <typename M>
using x = kiwi::unique_lock<M>;
template <typename M>
using s = kiwi::shared_lock<M>;
template <typename M>
using u = kiwi::upgrade_lock<M>;

template <typename M>
using xg = kiwi::unique_lock_guard<M>;
template <typename M>
using sg = kiwi::shared_lock_guard<M>;

template <typename>
struct lock_guard_lock_t_;
template <typename M>
struct lock_guard_lock_t_<kiwi::unique_lock_guard<M>> {
  using type = kiwi::unique_lock<M>;
};
template <typename M>
struct lock_guard_lock_t_<kiwi::shared_lock_guard<M>> {
  using type = kiwi::shared_lock<M>;
};
template <typename LG>
using lock_guard_lock_t = typename lock_guard_lock_t_<LG>::type;
template <typename LG>
using lock_guard_state_t =
    kiwi::detail::lock_state_type_of_t<lock_guard_lock_t<LG>>;
...
struct LockTest : testing::TestWithParam<Param> {
  template <typename... A>
  using deduction_unique = decltype(std::unique_lock{KIWI_DECLVAL(A)...});
  template <typename... A>
  using deduction_shared = decltype(std::shared_lock{KIWI_DECLVAL(A)...});
  template <typename... A>
  using deduction_upgrade = decltype(kiwi::upgrade_lock{KIWI_DECLVAL(A)...});
  ...
  static constexpr void check_deductions() {
    ...
    static_assert_same<std::unique_lock<mutex_type>,
                       deduction_unique<mutex_type&>>();
    static_assert_same<std::shared_lock<mutex_type>,
                       deduction_shared<mutex_type&>>();
    static_assert_same<kiwi::upgrade_lock<mutex_type>,
...
REGISTER_TYPED_TEST_SUITE_P(
    LockTest, ctor, construct_default, construct_mutex, construct_mutex_defer,
    construct_mutex_try_pass, construct_mutex_try_fail, construct_mutex_adopt,
    construct_mutex_duration_pass, construct_mutex_duration_fail,
    construct_mutex_time_point_pass, construct_mutex_time_point_fail,
    move_construct, destruct, move_assign, lock_pass, lock_fail, lock_owns,
    lock_empty, try_lock_pass, try_lock_fail, try_lock_owns, try_lock_empty,
    try_lock_for_pass, try_lock_for_fail, try_lock_for_owns, try_lock_for_empty,
    try_lock_until_pass, try_lock_until_fail, try_lock_until_owns,
    try_lock_until_empty, unlock_owns, unlock_unlocked, unlock_unowned,
    unlock_empty, release_empty, release_unowned, release_owns, swap_);

INSTANTIATE_TYPED_TEST_SUITE_P(x1, LockTest,
                               decltype(LockTestParam<0, 0, 0, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(X2, LockTest,
                               decltype(LockTestParam<1, 0, 0, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(s1, LockTest,
                               decltype(LockTestParam<0, 0, 0, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(S2, LockTest,
                               decltype(LockTestParam<0, 1, 0, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(u1, LockTest,
                               decltype(LockTestParam<0, 0, 0, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(U2, LockTest,
                               decltype(LockTestParam<0, 0, 1, u>{}));
...
REGISTER_TYPED_TEST_SUITE_P(LockGuardTest,  //
                            construct_mutex, construct_mutex_adopt);

INSTANTIATE_TYPED_TEST_SUITE_P(x1, LockGuardTest,
                               decltype(LockGuardTestParam<0, 0, 0, xg>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(X2, LockGuardTest,
                               decltype(LockGuardTestParam<1, 0, 0, xg>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(s1, LockGuardTest,
                               decltype(LockGuardTestParam<0, 0, 0, sg>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(S2, LockGuardTest,
                               decltype(LockGuardTestParam<0, 1, 0, sg>{}));
...
template <int X, int S, int U, template <typename> class FromL,
...
TYPED_TEST_P(TransitionLockTest, transition) {
  ...
  to_lock_type l1 = kiwi::transition_lock<TypeParam::template to_lock_type>(l0);
  EXPECT_TRUE(l1.owns_lock());
  EXPECT_FALSE(l0.owns_lock());
...
INSTANTIATE_TYPED_TEST_SUITE_P(xs1, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, x, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Xs2, TransitionLockTest,
                               decltype(XLockTestParam<1, 0, 0, x, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(xS3, TransitionLockTest,
                               decltype(XLockTestParam<0, 1, 0, x, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(XS4, TransitionLockTest,
                               decltype(XLockTestParam<1, 1, 0, x, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(xu1, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, x, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Xu2, TransitionLockTest,
                               decltype(XLockTestParam<1, 0, 0, x, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(xU3, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 1, x, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(XU4, TransitionLockTest,
                               decltype(XLockTestParam<1, 0, 1, x, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(us1, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, u, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Us2, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 1, u, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(uS3, TransitionLockTest,
                               decltype(XLockTestParam<0, 1, 0, u, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(US4, TransitionLockTest,
                               decltype(XLockTestParam<0, 1, 1, u, s>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(ux1, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, u, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Ux2, TransitionLockTest,
                               decltype(XLockTestParam<0, 0, 1, u, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(uX3, TransitionLockTest,
                               decltype(XLockTestParam<1, 0, 0, u, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(UX4, TransitionLockTest,
                               decltype(XLockTestParam<1, 0, 1, u, x>{}));
...
TYPED_TEST_P(TryTransitionLockTest, try_transition) {
  ...
  EXPECT_EQ(held_v<from_lock_type>, m.held);
  to_lock_type l1 =
      kiwi::try_transition_lock<TypeParam::template to_lock_type>(l0);
  EXPECT_TRUE(l1.owns_lock());
  EXPECT_FALSE(l0.owns_lock());
  EXPECT_EQ(held_v<to_lock_type>, m.held);
}
...
TYPED_TEST_P(TryTransitionLockTest, try_transition_for) {
  ...
  EXPECT_EQ(held_v<from_lock_type>, m.held);
  to_lock_type l1 =
      kiwi::try_transition_lock_for<TypeParam::template to_lock_type>(l0, 1s);
  EXPECT_TRUE(l1.owns_lock());
  EXPECT_FALSE(l0.owns_lock());
  EXPECT_EQ(held_v<to_lock_type>, m.held);
}
...
TYPED_TEST_P(TryTransitionLockTest, try_transition_until) {
  ...
  EXPECT_EQ(held_v<from_lock_type>, m.held);
  to_lock_type l1 =
      kiwi::try_transition_lock_until<TypeParam::template to_lock_type>(
          l0, m.now + 1s);
  EXPECT_TRUE(l1.owns_lock());
  EXPECT_FALSE(l0.owns_lock());
  EXPECT_EQ(held_v<to_lock_type>, m.held);
...
REGISTER_TYPED_TEST_SUITE_P(TryTransitionLockTest, try_transition,
                            try_transition_for, try_transition_until);

INSTANTIATE_TYPED_TEST_SUITE_P(su1, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, s, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Su2, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 1, 0, s, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(sU3, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 0, 1, s, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(SU4, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 1, 1, s, u>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(sx1, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, s, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Sx2, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 1, 0, s, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(sX3, TryTransitionLockTest,
                               decltype(XLockTestParam<1, 0, 0, s, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(SX4, TryTransitionLockTest,
                               decltype(XLockTestParam<1, 1, 0, s, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(ux1, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 0, 0, u, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(Ux2, TryTransitionLockTest,
                               decltype(XLockTestParam<0, 0, 1, u, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(uX3, TryTransitionLockTest,
                               decltype(XLockTestParam<1, 0, 0, u, x>{}));
INSTANTIATE_TYPED_TEST_SUITE_P(UX4, TryTransitionLockTest,
                               decltype(XLockTestParam<1, 0, 1, u, x>{}));

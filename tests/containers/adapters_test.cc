...
[[maybe_unused]] void StaticAsserts() {
  {
    ...
    std::vector<int> v;
    static_assert(std::ranges::range<decltype(kiwi::Reversed(v))>);
    static_assert(std::ranges::sized_range<decltype(kiwi::Reversed(v))>);
    // `kiwi::Reversed()` takes a const ref to the vector, which is, by
    // definition, a borrowed range.
    static_assert(std::ranges::borrowed_range<decltype(kiwi::Reversed(v))>);

    auto make_vector = [] { return std::vector<int>(); };
    static_assert(std::ranges::range<decltype(kiwi::Reversed(make_vector()))>);
    static_assert(
        std::ranges::sized_range<decltype(kiwi::Reversed(make_vector()))>);
    static_assert(
        !std::ranges::borrowed_range<decltype(kiwi::Reversed(make_vector()))>);
  }

  {
    kiwi::span<int> s;
    static_assert(std::ranges::range<decltype(kiwi::Reversed(s))>);
    static_assert(std::ranges::sized_range<decltype(kiwi::Reversed(s))>);
    static_assert(std::ranges::borrowed_range<decltype(kiwi::Reversed(s))>);

    auto rvalue_span = [] { return kiwi::span<int>(); };
    static_assert(std::ranges::range<decltype(kiwi::Reversed(rvalue_span()))>);
    static_assert(
        std::ranges::sized_range<decltype(kiwi::Reversed(rvalue_span()))>);
    static_assert(
        std::ranges::borrowed_range<decltype(kiwi::Reversed(rvalue_span()))>);
  }

...
using TypedTree = flat_tree<typename ContainerT::value_type, std::identity,
                            std::less<>, ContainerT>;
using IntTree = TypedTree<std::vector<int>>;
using IntPair = std::pair<int, int>;
using IntPairTree = flat_tree<IntPair, std::identity, LessByFirst<IntPair>,
                              std::vector<IntPair>>;
using MoveOnlyTree = flat_tree<MoveOnlyInt, std::identity, std::less<>,
                               std::vector<MoveOnlyInt>>;
using EmplaceableTree = flat_tree<Emplaceable, std::identity, std::less<>,
                                  std::vector<Emplaceable>>;
using ReversedTree =
    flat_tree<int, std::identity, std::greater<>, std::vector<int>>;

using TreeWithStrangeCompare =
    flat_tree<int, std::identity, NonDefaultConstructibleCompare,
              std::vector<int>>;
...
TYPED_TEST_P(FlatTreeTest, EraseIf) {
  ...
  EXPECT_EQ(0u, kiwi::EraseIf(x, [](int) { return false; }));
  EXPECT_THAT(x, ElementsAre());

  x = {1, 2, 3};
  EXPECT_EQ(1u, kiwi::EraseIf(x, [](int elem) { return !(elem & 1); }));
  EXPECT_THAT(x, ElementsAre(1, 3));

  x = {1, 2, 3, 4};
  EXPECT_EQ(2u, kiwi::EraseIf(x, [](int elem) { return elem & 1; }));
...
TYPED_TEST_P(FlatTreeTest, SortedUniqueRangeConstructorDCHECKs) {
  int unsorted[] = {2, 1};
  EXPECT_DEATH(TypedTree<TypeParam>(sorted_unique, std::begin(unsorted),
                                    std::end(unsorted)),
               "");

  int repeated[] = {1, 2, 2};
  EXPECT_DEATH(TypedTree<TypeParam>(sorted_unique, std::begin(repeated),
                                    std::end(repeated)),
               "");
}
...
TYPED_TEST_P(FlatTreeTest, SortedUniqueVectorCopyConstructorDCHECKs) {
  ...
  EXPECT_DEATH(TypedTree<TypeParam>(sorted_unique, unsorted), "");

  TypeParam repeated = {1, 2, 2};
  EXPECT_DEATH(TypedTree<TypeParam>(sorted_unique, repeated), "");
}

TYPED_TEST_P(FlatTreeTest, SortedUniqueVectorMoveConstructorDCHECKs) {
  TypeParam unsorted = {2, 1};
  EXPECT_DEATH(TypedTree<TypeParam>(sorted_unique, std::move(unsorted)), "");

  TypeParam repeated = {1, 2, 2};
  EXPECT_DEATH(TypedTree<TypeParam>(sorted_unique, std::move(repeated)), "");
}
...
TYPED_TEST_P(FlatTreeTest, ReplaceDCHECKs) {
  TypedTree<TypeParam> tree;
  TypeParam unsorted = {2, 1};
  EXPECT_DEATH(tree.replace(std::move(unsorted)), "");

  TypeParam repeated = {1, 2, 2};
  EXPECT_DEATH(tree.replace(std::move(repeated)), "");
}

REGISTER_TYPED_TEST_SUITE_P(
    FlatTreeTest, DefaultConstructor, CopyConstructor, ContainerCopyConstructor,
    InitializerListConstructor, SortedUniqueContainerCopyConstructor,
    SortedUniqueInitializerListConstructor, CopyAssignable,
    InitializerListAssignable, Clear, Size, Empty, Iterators, InsertLValue,
    InsertPositionLValue, Emplace, EmplacePosition, Extract, Replace,
    ErasePosition, EraseRange, EraseKey, EraseEndDeath, Count, Find, Contains,
    EqualRange, LowerBound, UpperBound, Swap, EraseIf,
    SortedUniqueRangeConstructorDCHECKs,
    SortedUniqueVectorCopyConstructorDCHECKs,
    SortedUniqueVectorMoveConstructorDCHECKs,
    SortedUniqueInitializerListConstructorDCHECKs, ReplaceDCHECKs);

...
class DestructCounter {
 ...
 private:
  size_t* where_ = nullptr;
};

extern "C" void CStyleInvoker(void (*cb)(void*), void* arg) { (*cb)(arg); }
...
TEST(HeapArray, MoveConstructor) {
  ...
  kiwi::HeapArray<uint32_t> vec(std::move(that));
  EXPECT_EQ(vec.size(), 2u);
  EXPECT_NE(vec.data(), nullptr);
...
TEST(HeapArrayDeathTest, TakeFirstWithOverSize) {
  auto that = HeapArray<uint32_t>::WithSize(2u);
  EXPECT_DEATH(std::move(that).take_first(3u), "");
}

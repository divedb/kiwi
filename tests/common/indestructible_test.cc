...
#include "kiwi/common/indestructible.hh"
...
template <typename T>
struct DeferredDtor {
  kiwi::Indestructible<T>& obj_;
  explicit constexpr DeferredDtor(kiwi::Indestructible<T>& obj) noexcept
      : obj_{obj} {}
  ~DeferredDtor() { obj_->~T(); }
};
...
TEST_F(IndestructibleTest, list_initialization) {
  kiwi::Indestructible<std::map<int, int>> map{{{1, 2}}};
  DeferredDtor s{map};
...
TEST_F(IndestructibleTest, conversion) {
  ...
  kiwi::Indestructible<I> map{I{{"foo", "bar"}}};
  DeferredDtor s{map};
  I& r = map;
...
TEST_F(IndestructibleTest, factory_method) {
  ...
  kiwi::Indestructible<Foo> foo(kiwi::factory_constructor, factory);

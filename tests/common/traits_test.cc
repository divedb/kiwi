...
TEST(Traits, type_identity) {
  EXPECT_TRUE((std::is_same_v<int, kiwi::type_identity_t<int>>));
  EXPECT_TRUE((std::is_same_v<int, kiwi::type_identity<int>::type>));
  EXPECT_TRUE(
...
struct Cond {
  ...
  static auto fun_std(std::conditional_t<V, K, std::string>&& arg) {
    return std::is_same<kiwi::remove_cvref_t<decltype(arg)>, std::string>{};
  }
  template <typename K = std::string>
  static auto fun_kiwi(kiwi::conditional_t<V, K, std::string>&& arg) {
    return std::is_same<kiwi::remove_cvref_t<decltype(arg)>, std::string>{};
  }
};

TEST(Traits, conditional) {
  using kiwi::conditional_t;
  ...
  EXPECT_TRUE(Cond<false>::fun_kiwi("hello"));
  EXPECT_FALSE(Cond<true>::fun_kiwi("hello"));
}
...
TEST(Traits, isNegative) {
  EXPECT_TRUE(kiwi::is_negative(-1));
  EXPECT_FALSE(kiwi::is_negative(0));
  EXPECT_FALSE(kiwi::is_negative(1));
  EXPECT_FALSE(kiwi::is_negative(0u));
  EXPECT_FALSE(kiwi::is_negative(1u));

  EXPECT_TRUE(kiwi::is_non_positive(-1));
  EXPECT_TRUE(kiwi::is_non_positive(0));
  EXPECT_FALSE(kiwi::is_non_positive(1));
  EXPECT_TRUE(kiwi::is_non_positive(0u));
  EXPECT_FALSE(kiwi::is_non_positive(1u));
}
...
TEST(Traits, relational) {
  ...
  EXPECT_FALSE((kiwi::less_than<uint8_t, 0u, uint8_t>(0u)));
  EXPECT_FALSE((kiwi::less_than<uint8_t, 0u, uint8_t>(254u)));
  EXPECT_FALSE((kiwi::less_than<uint8_t, 255u, uint8_t>(255u)));
  EXPECT_TRUE((kiwi::less_than<uint8_t, 255u, uint8_t>(254u)));

  // Making sure signed to unsigned comparisons are not truncated.
  EXPECT_TRUE((kiwi::less_than<uint8_t, 0, int8_t>(-1)));
  EXPECT_TRUE((kiwi::less_than<uint16_t, 0, int16_t>(-1)));
  EXPECT_TRUE((kiwi::less_than<uint32_t, 0, int32_t>(-1)));
  EXPECT_TRUE((kiwi::less_than<uint64_t, 0, int64_t>(-1)));

  EXPECT_FALSE((kiwi::less_than<int8_t, -1, uint8_t>(0)));
  EXPECT_FALSE((kiwi::less_than<int16_t, -1, uint16_t>(0)));
  EXPECT_FALSE((kiwi::less_than<int32_t, -1, uint32_t>(0)));
  EXPECT_FALSE((kiwi::less_than<int64_t, -1, uint64_t>(0)));

  EXPECT_FALSE((kiwi::greater_than<uint8_t, 0u, uint8_t>(0u)));
  EXPECT_TRUE((kiwi::greater_than<uint8_t, 0u, uint8_t>(254u)));
  EXPECT_FALSE((kiwi::greater_than<uint8_t, 255u, uint8_t>(255u)));
  EXPECT_FALSE((kiwi::greater_than<uint8_t, 255u, uint8_t>(254u)));

  EXPECT_FALSE((kiwi::greater_than<uint8_t, 0, int8_t>(-1)));
  EXPECT_FALSE((kiwi::greater_than<uint16_t, 0, int16_t>(-1)));
  EXPECT_FALSE((kiwi::greater_than<uint32_t, 0, int32_t>(-1)));
  EXPECT_FALSE((kiwi::greater_than<uint64_t, 0, int64_t>(-1)));

  EXPECT_TRUE((kiwi::greater_than<int8_t, -1, uint8_t>(0)));
  EXPECT_TRUE((kiwi::greater_than<int16_t, -1, uint16_t>(0)));
  EXPECT_TRUE((kiwi::greater_than<int32_t, -1, uint32_t>(0)));
  EXPECT_TRUE((kiwi::greater_than<int64_t, -1, uint64_t>(0)));
}
...
TEST(Traits, int128) {
  EXPECT_TRUE(
      (::std::is_same<kiwi::make_unsigned_t<int128_t>, uint128_t>::value));
  EXPECT_TRUE((::std::is_same<kiwi::make_signed_t<int128_t>, int128_t>::value));
  EXPECT_TRUE(
      (::std::is_same<kiwi::make_unsigned_t<uint128_t>, uint128_t>::value));
  EXPECT_TRUE(  //
      (::std::is_same<kiwi::make_signed_t<uint128_t>, int128_t>::value));
  EXPECT_TRUE((kiwi::is_arithmetic_v<int128_t>));
  EXPECT_TRUE((kiwi::is_arithmetic_v<uint128_t>));
  EXPECT_TRUE((kiwi::is_integral_v<int128_t>));
  EXPECT_TRUE((kiwi::is_integral_v<uint128_t>));
  EXPECT_FALSE((kiwi::is_unsigned_v<int128_t>));
  EXPECT_TRUE((kiwi::is_signed_v<int128_t>));
  EXPECT_TRUE((kiwi::is_unsigned_v<uint128_t>));
  EXPECT_FALSE((kiwi::is_signed_v<__uint128_t>));
}
...
template <class T>
struct has_value_type<T, kiwi::void_t<typename T::value_type>>
    : std::true_type {};
...
struct container {
  template <class... Args>
  container(kiwi::type_t<some_tag, decltype(T(std::declval<Args>()...))>,
            Args&&...) {}
};
}  // namespace

TEST(Traits, voidT) {
  EXPECT_TRUE((::std::is_same<kiwi::void_t<>, void>::value));
  EXPECT_TRUE((::std::is_same<kiwi::void_t<int>, void>::value));
  EXPECT_TRUE((::std::is_same<kiwi::void_t<int, short>, void>::value));
  EXPECT_TRUE(
      (::std::is_same<kiwi::void_t<int, short, std::string>, void>::value));
  EXPECT_TRUE((::has_value_type<std::string>::value));
  EXPECT_FALSE((::has_value_type<int>::value));
}

TEST(Traits, typeT) {
  EXPECT_TRUE((::std::is_same<kiwi::type_t<float>, float>::value));
  EXPECT_TRUE((::std::is_same<kiwi::type_t<float, int>, float>::value));
  EXPECT_TRUE((::std::is_same<kiwi::type_t<float, int, short>, float>::value));
  EXPECT_TRUE((::std::is_same<kiwi::type_t<float, int, short, std::string>,
                              float>::value));
  EXPECT_TRUE((::std::is_constructible<::container<std::string>, some_tag,
                                       std::string>::value));
  EXPECT_FALSE((::std::is_constructible<::container<std::string>, some_tag,
                                        float>::value));
}
...
TEST(Traits, detectedOrT) {
  EXPECT_TRUE((  //
      std::is_same<kiwi::detected_or_t<float, detector_find, std::string, char>,
                   std::string::size_type>::value));
  EXPECT_TRUE((  //
      std::is_same<kiwi::detected_or_t<float, detector_find, double, char>,
                   float>::value));
}

TEST(Traits, detectedT) {
  EXPECT_TRUE((  //
      std::is_same<kiwi::detected_t<detector_find, std::string, char>,
                   std::string::size_type>::value));
  EXPECT_TRUE((  //
      std::is_same<kiwi::detected_t<detector_find, double, char>,
                   kiwi::nonesuch>::value));
}

TEST(Traits, isDetected) {
  EXPECT_TRUE((kiwi::is_detected<detector_find, std::string, char>::value));
  EXPECT_FALSE((kiwi::is_detected<detector_find, double, char>::value));
}

TEST(Traits, isDetectedV) {
  EXPECT_TRUE((kiwi::is_detected_v<detector_find, std::string, char>));
  EXPECT_FALSE((kiwi::is_detected_v<detector_find, double, char>));
}

TEST(Traits, fallbackIsNothrowConvertible) {
  EXPECT_FALSE((kiwi::fallback::is_nothrow_convertible<int, void>::value));
  EXPECT_TRUE((kiwi::fallback::is_nothrow_convertible<void, void>::value));
  struct foo {
    /* implicit */ [[maybe_unused]] operator std::false_type();
    /* implicit */ [[maybe_unused]] operator std::true_type() noexcept;
  };
  EXPECT_FALSE(
      (kiwi::fallback::is_nothrow_convertible<foo, std::false_type>::value));
  EXPECT_TRUE(
      (kiwi::fallback::is_nothrow_convertible<foo, std::true_type>::value));
}

TEST(Traits, isNothrowConvertible) {
  EXPECT_FALSE((kiwi::is_nothrow_convertible<int, void>::value));
  EXPECT_TRUE((kiwi::is_nothrow_convertible<void, void>::value));
  struct foo {
    /* implicit */ [[maybe_unused]] operator std::false_type();
    /* implicit */ [[maybe_unused]] operator std::true_type() noexcept;
  };
  EXPECT_FALSE((kiwi::is_nothrow_convertible<foo, std::false_type>::value));
  EXPECT_TRUE((kiwi::is_nothrow_convertible<foo, std::true_type>::value));
}
...
TEST(Traits, removeCvref) {
  using kiwi::remove_cvref;
  using kiwi::remove_cvref_t;
...
TEST(Traits, like) {
  ...
  EXPECT_TRUE((std::is_same<like_t<int const volatile, char>,
...
TEST(Traits, isUnboundedArrayV) {
  EXPECT_FALSE((kiwi::is_unbounded_array_v<void>));
  EXPECT_FALSE((kiwi::is_unbounded_array_v<int>));
  EXPECT_TRUE((kiwi::is_unbounded_array_v<int[]>));
  EXPECT_FALSE((kiwi::is_unbounded_array_v<int[1]>));
}

TEST(Traits, isBoundedArrayV) {
  EXPECT_FALSE((kiwi::is_bounded_array_v<void>));
  EXPECT_FALSE((kiwi::is_bounded_array_v<int>));
  EXPECT_FALSE((kiwi::is_bounded_array_v<int[]>));
  EXPECT_TRUE((kiwi::is_bounded_array_v<int[1]>));
}
...
TEST(Traits, member_pointer_traits_data) {
  ...
  using d = float;
  using mp_do = float o::*;
  using mp_traits_do = kiwi::member_pointer_traits<mp_do>;
...
TEST(Traits, member_pointer_traits_function) {
  ...
  using mp_traits_fo = kiwi::member_pointer_traits<mp_fo>;
...
TEST(Traits, type_pack_element) {
  ...
  EXPECT_TRUE((  //
      std::is_same_v<test::fallback<3, int, int, int, double, int, int>,
                     double>));
  ...
  EXPECT_TRUE((                                                         //
      std::is_same_v<test::native<3, int, int, int, double, int, int>,  //
                     double>));
...
TEST(Traits, type_list_element) {
  EXPECT_TRUE((  //
      std::is_same_v<kiwi::type_list_element_t<
                         3, kiwi::tag_t<int, int, int, double, int, int>>,  //
                     double>));
  EXPECT_TRUE((  //
      std::is_same_v<kiwi::type_list_element_t<0, kiwi::tag_t<int[1]>>,
                     int[1]>));
}
